...
class CodeGenContext {
 ...
 public:
  ...
  BasicBlock* currentBlock() const { return blockStack.back()->block; }
  ...
  Value* getCurrentReturnValue() { return blockStack.back()->returnValue; }
  ...
  void setArraySize(string name, std::vector<uint64_t> value) {
    ...
    //        cout << "blockStack.back()->arraySizes.size()" <<
    //        blockStack.back()->arraySizes.size() << endl;
  }
  ...
  void PrintSymTable() const {
    ...
    for (auto it = blockStack.begin(); it != blockStack.end(); it++) {
      for (auto it2 = (*it)->locals.begin(); it2 != (*it)->locals.end();
           it2++) {
        cout << prefix << it2->first << " = " << it2->second << ": "
             << this->getSymbolType(it2->first) << endl;
      }
      prefix += "\t";
    }
    cout << "===================================" << endl;
