...
void TypeSystem::addStructMember(string structName, string memType,
                                 string memName) {
...
Type *TypeSystem::getVarType(const NIdentifier &type) {
  assert(type.isType);
  if (type.isArray) {  // array type when allocation, pointer type when pass
                       // parameters
                       //        return ArrayType::get(getVarType(type.name),
                       //        type.arraySize->value);
    return PointerType::get(getVarType(type.name), 0);
    //        PointerType::getUnqual(ArrayType::get(getVarType(type.name),
    //        type.arraySize->value));
  }
